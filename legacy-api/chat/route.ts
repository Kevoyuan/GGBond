...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 1. Log Session & User Message (wrapped with runNonBlockingAsync to avoid blocking event loop)
    try {
      ...
      const info = await runNonBlockingAsync(() => insertSessionFn());
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const processQueuedConfirmations = async () => {
          ...
          try {
            const ids = Array.from(pendingConfirmations);
        ...
        try {
          ...
          const unsubscribeConfirmation = coreWithConfirmation.subscribeConfirmationRequests((request) => {
            ...
            pendingConfirmations.add(request.correlationId);
            safeEnqueue({
          ...
          try {
            ...
            await runNonBlockingAsync(() => assistantInsertTx());
          } catch (e) {
          ...
          // Wrap persistUndoSnapshot and batch insert with runNonBlockingAsync to avoid blocking
          persistUndoSnapshot();
          ...
          if (pendingToolStats.length > 0 || pendingFileOps.length > 0) {
            try {
              ...
              await runNonBlockingAsync(() => batchInsertTx());
            } catch (err) {
