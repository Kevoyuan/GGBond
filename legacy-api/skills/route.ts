...
const getProjectSkillRoots = () => {
  const workspaceRoot = resolveDefaultWorkspaceRoot();
  return [
    path.resolve(path.join(workspaceRoot, '.gemini', 'skills')),
    path.resolve(path.join(workspaceRoot, '.agent', 'skills')),
    path.resolve(path.join(workspaceRoot, '.agents', 'skills')),
  ];
};
