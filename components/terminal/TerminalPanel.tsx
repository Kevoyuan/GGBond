...
import { TerminalEnvironmentDialog } from './TerminalEnvironmentDialog';
...
import {
  ...
  quoteForPosixShell,
  quoteForCmdShell,
  parseSimpleCdCommand,
  buildCdProbeCommand,
  parseResolvedCwdFromStdout,
  generateActionId,
  clampHeight,
  clampSidebarWidth,
...
export const TerminalPanel = React.memo(function TerminalPanel({
  ...
  // Buffering state for optimized terminal writes
  const writeBufferRef = useRef<Record<string, string>>({});
  const rafIdRef = useRef<Record<string, number | null>>({});
  ...
  const writeToTerminal = useCallback((sessionId: string, chunk: string) => {
    ...
    if (!writeBufferRef.current[sessionId]) {
      writeBufferRef.current[sessionId] = '';
    }
    writeBufferRef.current[sessionId] += chunk;

    if (rafIdRef.current[sessionId]) return;

    rafIdRef.current[sessionId] = requestAnimationFrame(() => {
      const pendingChunk = writeBufferRef.current[sessionId];
      if (pendingChunk) {
        term.write(pendingChunk);
        writeBufferRef.current[sessionId] = '';
      }
      rafIdRef.current[sessionId] = null;
    });
  ...
  }, [onHeightChange, panelHeight, variant]);
  ...
  useEffect(() => {
    sessions.forEach((session) => {
      ...
      try {
        ...
        console.warn('Failed to load WebGL Addon for xterm', err);
      }
  ...
  }, [isResizing, isSidebarResizing, variant]);
  ...
  const handleRunCommand = useCallback(
    async (sessionId: string, commandOverride?: string) => {
      ...
      const externalSessionId = typeof chatSessionId === 'string' && chatSessionId.trim() ? chatSessionId : null;
  ...
  return (
    <>
      <div
        className={cn(
          "relative bg-background/80 dark:bg-[#020617]/80 backdrop-blur-md flex flex-col transform-gpu",
          variant === 'bottom' ? "border-t border-border shadow-2xl" : "h-full border-0 shadow-none",
          !isResizing && variant === 'bottom' && "transition-[height] duration-200 ease-in-out will-change-[height]"
        )}
        style={{ height: variant === 'bottom' ? `${panelHeight}px` : '100%', contain: 'layout style' }}
      >
        {variant === 'bottom' && (
          <div
            className={cn(
